...
export function useDashboardData() {
	...
	const paymentsQuery = useQuery({
		...
		queryFn: () => listPaymentsForBookings((bookingsQuery.data || []).map(b => b.id)),
		queryKey: ['payments', userId, JSON.stringify((bookingsQuery.data || []).map(b => b.id))]
	});

	const markReadMutation = useMutation({
		mutationFn: (id: number) => markNotificationAsRead(id),
		onError: (error) => showFeedback({ description: getErrorMessage(error), title: 'Notification update failed', variant: 'destructive' }),
		onSuccess: () => {
	...
	const facilityNameById = useMemo(() => 
		new Map((facilitiesQuery.data || []).map(f => [f.id, f.name])), 
	[facilitiesQuery.data]);

	const visibleBookings = useMemo(() => 
		(bookingsQuery.data || []).filter(b => bookingFilter === 'ALL' || b.status === bookingFilter),
